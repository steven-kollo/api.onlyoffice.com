...
<h2 id="install" class="copy-link"><span class="style_step">步骤1. </span>安装必备组件并运行带编辑器的网站</h2>
<p>PHP示例提供了各种安装选项，但我们强烈建议使用Docker。</p>
...
<p>然后打开示例目录和<a href="https://github.com/ONLYOFFICE/document-server-integration/blob/880423545fc0c1f21b004e1c8121a319809f4b72/web/documentserver-example/php/Makefile#L60" target="_blank">容器</a>:</p>
<pre>
cd "PHP Example"
make compose-prod
</pre>
<p>默认情况下，服务器从 <em>localhost:80</em> 启动。</p>
<p>要配置示例，您可以编辑<a href="https://github.com/ONLYOFFICE/document-server-integration/blob/880423545fc0c1f21b004e1c8121a319809f4b72/web/documentserver-example/php/docker-compose.yml" target="_blank">docker-compose.yml</a>。
有关环境变量的更多信息，请参阅<a href="#configure">下面</a>。</p>

<p><b>选项 2. 在本地计算机上</b></p>
<p>在深入研究示例之前，您需要安装 ONLYOFFICE Docs。
查看详细指南，了解如何在 <a href="https://helpcenter.onlyoffice.com/installation/docs-developer-install-windows.aspx?from=api_php_example" target="_blank">Windows</a>，
...
或 <a href="https://helpcenter.onlyoffice.com/installation/docs-developer-install-docker.aspx?from=api_php_example" target="_blank">Docker</a>上安装它。</p>
<p>要在本地计算机上运行该示例，您需要 <a href="https://www.php.net/" target="_blank">PHP 8.2.11</a>
与<a href="https://getcomposer.org/" target="_blank">Composer 2.6.5</a>。 此外，您可能需要考虑安装 <a href="https://www.gnu.org/software/make/" target="_blank">GNU Make 4.4.1</a>，尽管它是可选的。
这些是工具所需的最低版本。</p>
<p>安装完所有内容后，下载发布的压缩文件并将其解压：</p>
<pre>
curl --output PHP.Example.zip --location https://github.com/ONLYOFFICE/document-server-integration/releases/latest/download/PHP.Example.zip
...
<p>然后打开示例目录， <a href="https://github.com/ONLYOFFICE/document-server-integration/blob/880423545fc0c1f21b004e1c8121a319809f4b72/web/documentserver-example/php/Makefile#L16" target="_blank">安装依赖项</a>，
并<a href="https://github.com/ONLYOFFICE/document-server-integration/blob/880423545fc0c1f21b004e1c8121a319809f4b72/web/documentserver-example/php/Makefile#L40" target="_blank">启动服务器</a>:</p>
<pre>
cd "PHP Example"
make prod
make server-prod
</pre>
<p>默认情况下，服务器从 <em>0.0.0.0:9000</em> 启动。</p>
...
<h2 id="configure" class="copy-link"><span class="style_step">步骤3. </span>配置 PHP 示例</h2>
