...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <p class="dscr">该 <a href="https://github.com/ONLYOFFICE/onlyoffice-chamilo" target="_blank">插件</a> 让用户能够 <a href="https://chamilo.org/en/" target="_blank">在Chamilo</a> 中使用 ONLYOFFICE 文档编辑办公文档。</p>
    ...
    <p>
        您需要一个ONLYOFFICE 文档（文档服务器）实例，它可从 Chamilo 和任何客户终端解析和连接。
        ...
        ONLYOFFICE 文档还必须能够直接发布到 Chamilo。
    </p>
    <p>
        ONLYOFFICE 文档和 Chamilo 可以安装在不同的计算机上，也可以安装在同一台机器上。
        如果您使用一台机器，请为文档服务器设置自定义端口，因为默认情况下 ONLYOFFICE 文档和 Chamilo 都在端口 80 上工作。</p>
    <p>
...
zip onlyoffice.zip -r onlyoffice
    ...
    <p>要开始将 ONLYOFFICE 文档与 Chamilo 一起使用，必须执行以下步骤：</p>
    <ol>
    ...
    <p>
        ...
        为了正常工作，Chamilo 服务器地址也必须可以从 <b>ONLYOFFICE 文档</b> 访问。
    </p>
    <p>从 7.2 版开始，默认情况下启用 JWT，并自动生成密钥以限制对 ONLYOFFICE 文档的访问，这也是出于安全原因和数据完整性考虑。
        在 Chamilo <b>设置</b>页面上指定您自己的<b>密钥</b>。 在 ONLYOFFICE 文档 <a href="/editors/signature/">配置文件</a>中，指定相同的密钥并启用验证。</p>
    ...
    <ol>
        ...
        <li>
            ...
            <ul>
                ...
                <li><b>callbackUrl</b> - ONLYOFFICE 文档用于通知文档编辑状态的 URL；</li>
                <li><b>documentServerUrl</b> - 客户端需要响应 ONLYOFFICE 文档的 URL（可以在管理设置页面设​置）；</li>
                <li><b>key</b> - 指示 ONLYOFFICE 文档是否再次下载文档的电子标签。</li>
            </ul>
        </li>
        <li>服务器返回一个带有打开编辑器脚本的页面。</li>
        <li>浏览器打开此页面并加载编辑器。</li>
        <li>浏览器向 ONLYOFFICE 文档发出请求并将文档配置传递给它。</li>
        <li>ONLYOFFICE 文档加载文档，用户开始编辑。</li>
        <li>ONLYOFFICE 文档向 <em>callbackUrl</em> 发送 POST 请求，通知 Chamilo：用户正在编辑文档。</li>
        <li>当所有用户都完成编辑后，他们会关闭编辑器窗口。</li>
        <li><a href="<%= Url.Action("save") %>#savedelay">10 seconds</a>秒后，ONLYOFFICE 文档向 <em>callbackUrl</em> 发出 POST 请求，其中包含编辑已结束的信息，并发送新文档版本的链接。</li>
