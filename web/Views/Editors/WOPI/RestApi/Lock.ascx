...
<h1>
    ...
    <span class="hdr">锁定</span>
</h1>

    <div class="header-gray"><b>POST /wopi/files/<i>(file_id)</i></b></div>
    <p class="dscr">由请求锁定的online office锁定文件编辑。</p>
    <p>此操作的工作原理如下：</p>
    <ol>
        <li>WOPI 主机检查文件当前是否被锁定。</li>
        <li>如果文件 <b>未被锁定</b>，主机应该锁定文件并返回 <b>200 OK</b>。</li>
        <li>如果文件已经 <b>被锁定</b> 并且 <b>X-WOPI-Lock</b> 值与文件当前的锁定匹配，主机应该刷新锁定计时器并返回 <b>200 OK</b>。</li>
        <li>在所有其他情况下，主机必须返回 <b>409 Conflict</b> (<em>"lock mismatch"</em>) 并包含 <b>X-WOPI-Lock</b> 响应标头，标头中包含文件当前锁的值。</li>
        <li>在文件被第三方客户端锁定的情况下，主机仍应始终在 <b>X-WOPI-Lock</b> 响应标头中包含当前锁 ID。</li>
    </ol>
    ...
    <table class="table">
        ...
        <tbody>
            <tr>
                ...
                <td>来自 WOPI 服务器的请求操作 (<em>LOCK</em>)。</td>
                <td>string</td>
                <td>必需的</td>
            </tr>
            <tr>
