...
namespace ASC.Api.Web.Help.Helpers
{
    public static class Highliter
    {
        ...
        {
            var replace = Highlite.Replace(address, "<span class=\"bracket\">{</span><span class=\"param-name\">$1</span><span class=\"bracket\">}</span>");
        ...
        public static MvcHtmlString HighliteString(string toHighlite, string what)
        {
            ...
            return
                MvcHtmlString.Create(Regex.Replace(toHighlite, "(" +string.Join("|",words.Select(x=>Regex.Escape(x)).ToArray()) + ")",
                                                   "<span class=\"search-result\">$1</span>", RegexOptions.IgnoreCase | RegexOptions.Singleline));
        }
