...
export interface SyntaxHighlightParameters {
  syntax: string
  children: string
}

export interface ContextProperties {
  onHighlightSyntax({syntax, children}: SyntaxHighlightParameters): any
  onLink(t: Tokenizer.Token): string | undefined
  onProcessMarkdown({children}: JSX.ElementChildrenAttribute): any
  onRetrieve(r: Library.Reference): Library.Declaration | undefined
}

const Context = createContext(context())

function context(): ContextProperties {
  return {
    onHighlightSyntax() {
...
export function Root({declaration: d, ...ctx}: RootParameters): JSX.Element {
  return (
...
function Declaration({declaration: d}: DeclarationParameters): JSX.Element {
  switch (d.kind) {
  case "class":
...
function ClassDeclaration({declaration: d}: ClassDeclarationParameters): JSX.Element {
  return (<>
    <DeclarationTitle title={d.title} />
...
interface DeclarationTitleParameters {
  title: string
}

function DeclarationTitle({title}: DeclarationTitleParameters): JSX.Element {
  return <h1>{title}</h1>
}
